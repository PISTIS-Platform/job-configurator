...
def pistis_workflow_template():
    ...
    def periodic_group():    
        ...
        @task
        def build_conf():
            #conf={"logical_date": , "workflow": "{{ ti.xcom_pull(task_ids='periodic_group.build_periodic_workflow', key='return_value') }}", "access_token": "{{ ti.xcom_pull(task_ids='generate_conf_for_job_dag', key='return_value').job_data.access_token }}" },
            context = get_current_context()
            conf = {}
            ...
            if (len(dr_list) > 0):
                ...
                # Retrieve uuid and data_uuid from last job execution
    ...
    #get_job_from_workflow() >> generate_conf_for_job_dag() >> trigger_pistis_job >> get_current_workflow()
